<?php
   ...
   if(isset($_POST['submit'])) {
    ...
    $phone = $_POST['phone'];
    try {
      $statement = $pdo->prepare("UPDATE kupec SET Ime = ?, Priimek = ?, Naslov = ?, Telefonska_stevilka = ? WHERE ID = ?");
      $statement->bindValue(1, $name);
      $statement->bindValue(2, $surname);
      $statement->bindValue(3, $address);
      $statement->bindValue(4, $phone);
      $statement->bindValue(5, $_SESSION["user_ID"]);
      $statement->execute();
   ...
   if(isset($_POST['password_reset'])) {
    ...
    if ($old_password == $user["Password"]) {
      if ($new_password == $password_confirm) {
        try {
          $statement = $pdo->prepare("UPDATE kupec SET Password = ? WHERE ID = ?");
...
<head>
	...
	<title>Mobilko | Nastavitve računa</title>
      ...
      <div class="container-fluid" style="margin-top: 2em;">
        ...
        <form class="form-horizontal" method="post" role="form">
          ...
          <div class="form-group">
						...
						<div class="col-sm-3">
							<input class="form-control" id="address" name="address" placeholder="" type="text" value="<?php echo $user["Naslov"];?>">
						</div>
					</div>
          <div class="form-group">
						<label class="col-sm-5 control-label" for="phone">Telefon</label>
						<div class="col-sm-3">
							<input class="form-control" id="phone" name="phone" placeholder="" type="text" value="<?php echo $user["Telefonska_stevilka"];?>">
						</div>
					</div>
